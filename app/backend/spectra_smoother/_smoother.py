...
class SpectraSmoother:
    ...
    def _smooth_rows(
        ...
        spectrum and reused for all rows.  All-NaN rows (CleanData dead
        pixels) are detected upfront and preserved as NaN in the output.
        """
        n_spectra, n_spectral = row_stack.shape
        cleaned = np.empty_like(row_stack, dtype=np.float64)
        meta: dict[str, Any] = {"method": self.method}

        # Detect all-NaN rows (CleanData dead pixels) — skip them and
        # exclude them from any population-level statistics (e.g. auto-lam).
        nan_row_mask = np.all(np.isnan(row_stack), axis=1)
        has_nan = bool(np.any(nan_row_mask))
        valid_rows = row_stack[~nan_row_mask] if has_nan else row_stack
        ...
        if self.method == "savgol":
            ...
            for i in range(n_spectra):
                if has_nan and nan_row_mask[i]:
                    ...
                    cleaned[i] = wavelet_smooth_1d(
                        row_stack[i],
                        wavelet=self.wavelet,
                        level=self.wavelet_level,
                        threshold_mode=self.wavelet_threshold,
                    )
