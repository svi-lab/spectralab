...
def axis_is_absolute_wavenumber_cm(
    ...
    Works with both old dim-name convention ("raman_shift", "wavenumber", "nm")
    and the new convention where dim is always "spectral" and units/long_name
    attrs carry the meaning.
    """
    name_l = spectral_dim.lower()
    # Backward compat: old dim-name convention
    ...
    if "nm" in units or "µm" in units or "metre" in units or "meter" in units:
        return False
    # wdfkit assigns RamanShift → long_name="RamanShift", units="1/cm"
    # Absolute wavenumber → units="cm^-1"
    if "raman" in long_name or "shift" in long_name:
        return False
    if units == "1/cm":  # Raman shift unit from wdfkit SpectralAxisSpec
        return False
