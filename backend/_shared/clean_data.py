...
class CleanData:
    ...
    **3D** ``(ny, nx, n_channels)``: NaN-fills the spectral values of flagged
    pixels in place; the array shape ``(ny, nx, n_channels)`` is preserved.
    Removed pixel positions ``(y, x)`` are recorded in
    ``attrs["treatments"]``.
    ...
    def _handle_3d(
        ...
        warnings.warn(
            f"Oversaturated spectra detected in '{filename}': "
            f"{n_bad} pixel(s) NaN-filled (3D shape preserved).",
            UserWarning,
            stacklevel=3,
        )
        ...
        meta: dict[str, Any] = {
            "removed_count": n_bad,
            "action": "NaN-filled (3D shape preserved)",
            "y_dim": y_dim,
            "x_dim": x_dim,
            "removed_positions": removed_positions,
            "filename": filename,
        }
        return _set_treatment(da.copy(data=arr), meta)
