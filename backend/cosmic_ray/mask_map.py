...
def _per_wavelength_cutoff_relax_factors(
    ...
    """Factors in [relax_floor, 1] that lower the cutoff in noisy bands.

    Large per-channel noise → smaller factor → higher sensitivity there.
    """
...
def _limit_mask_runs_along_spectral_axis(
    ...
    """Shorten any contiguous True run along λ to at most max_channels,
    centred on the largest residual in that run."""
    if max_channels < 1:
        return mask
...
def _dilate_mask_along_spectral_axis(
    ...
    return morphology.dilation(mask.astype(bool), footprint=footprint)
...
def _strict_spatial_local_max_mask(
    ...
    Suppresses extended bright patches that are not spike-like spatially.
    """
    ...
    for k in range(nlam):
        sl = field[:, :, k]
        neigh_max = grey_dilation(sl, footprint=neighbour_footprint, mode="nearest")
...
def interpolate_cosmic_ray_regions_spectrally(
    ...
    """Inpaint repair_mask points by interpolation along λ.

    Reference curve is spatial_median_reference[y, x, :]; other channels
    keep the original preprocessed values.
    """
...
def correct_cosmic_rays_on_map_cube(
    ...
    *,
    ...
    Repair: interpolate masked λ from the spatial median reference; result
    is clamped to ≥ 0 before returning.
    """
    # Handle NaN pixels (defensive — CleanData uses zeros, but guard for other sources)
    flat_in = np.asarray(values, dtype=float)
    orig_map_shape = flat_in.shape
    flat_2d = flat_in.reshape(-1, flat_in.shape[-1])
    nan_pix = np.all(np.isnan(flat_2d), axis=1)
    ...
    relax_ch = _per_wavelength_cutoff_relax_factors(noise_ch, map_noisy_channel_relax_min)
    ...
    if map_max_spectral_repair_extent is not None:
        dilated = _limit_mask_runs_along_spectral_axis(
            dilated, residual, int(map_max_spectral_repair_extent)
        )
...
def correct_cosmic_rays_collection(
    ...
    *,
    ...
    internally; detection and repair run on ``(n_spectra, n_channels)``.

    Two detection passes are run.  Pass 2 recomputes the reference on
    data cleaned by pass 1 so that CRs do not contaminate the reference.

    Result is clamped to ≥ 0 before returning.
    """
    ...
    # Handle NaN spectra (defensive — CleanData uses zeros, but guard for other
    # sources); fill before processing so reference computation is not
    # contaminated, then restore NaN at the end.
    nan_spec_mask = np.all(np.isnan(flat), axis=1)
    ...
    core_mask_p1 = (residual > threshold * noise_ch[np.newaxis, :]) | zero_mask_flat

    # Pass 2: rebuild reference excluding flagged data
    if method == "median":
        ...
        if clean_idx.size < k + 2:
            warnings.warn(
                ...
                "Reference unchanged — detection may be suboptimal.",
                UserWarning,
                stacklevel=4,
            )
        else:
    ...
    # Repair: interpolate from original spectrum's clean channels
    corrected = flat.copy()
    for r in range(n_spectra):
        m = dilated_flat[r]
        if np.any(m):
    ...
    corrected_out = np.maximum(corrected.reshape(orig_shape), 0.0)
    if has_nan_coll:
        corrected_out.reshape(-1, n_channels)[nan_spec_mask] = np.nan
