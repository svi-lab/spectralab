...
def axis_is_absolute_wavenumber_cm(
    ...
    long_name = str(da[spectral_dim].attrs.get("long_name", "")).lower()

    if "nm" in units or "µm" in units or "metre" in units or "meter" in units:
        return False
    if "raman" in long_name or "shift" in long_name:
        return False
    if units == "1/cm":
        return False
    ...
    if name_l == "wavenumber":
        return True
    return "cm^-1" in units or "cm-1" in units or "wavenumber" in long_name
...
def remove_harmonic_notches_from_spectrum_1d(
    ...
    *,
    ...
    """Notch ~1 nm around each harmonic peak using 1D linear interpolation.

    Always uses the 1D masking logic regardless of the parent DataArray shape.
    """
...
def harmonic_correct_dataarray(
    ...
    *,
    ...
    """If laser_wavelength_nm is ~355 nm, notch laser harmonics on every slice.

    Harmonic correction always uses the 1D masking logic: each spectrum is
    processed independently via remove_harmonic_notches_from_spectrum_1d.
