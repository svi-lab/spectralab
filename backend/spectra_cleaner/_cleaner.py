...
class Denoiser:
    ...
    **1-D single spectrum** ``(spectral,)`` or any input when
    ...
    def denoise(self, spectra: xr.DataArray) -> xr.DataArray:
    ...
    def denoise_with_decomposition(
        ...
        """Like :meth:`denoise`, but also returns the PCA decomposition.
    ...
    def transform(self, spectra: xr.DataArray) -> xr.DataArray:
        """Alias of :meth:`denoise`."""
        return self.denoise(spectra)
    ...
    def _clean_core(
        ...
        *,
        ...
        if n_spectra < 2:
            raise ValueError(
                "Denoiser needs more than one spectrum (PCA on a "
