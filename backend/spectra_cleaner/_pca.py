...
def denoise_spectra_pca(
    ...
    *,
    ...
    # Detect NaN rows (dead pixels NaN-filled by CleanData) before any
    # processing — sklearn PCA does not accept NaN natively.
    nan_row_mask = np.all(np.isnan(row_stack), axis=1)
    has_nan = bool(np.any(nan_row_mask))
    valid_idx = ~nan_row_mask

    # Subtract per-spectrum minimum only from valid rows; NaN rows keep 0
    # so they don't produce warnings and don't affect the min computation.
    per_spec_min = np.zeros((n_spectra, 1), dtype=float)
    if subtract_min and valid_idx.any():
        per_spec_min[valid_idx] = row_stack[valid_idx].min(axis=-1, keepdims=True)
    spectra_for_fit = row_stack - per_spec_min

    if has_nan:
        # Replace NaN rows with the median of valid spectra so PCA sees a
        # full matrix.  These rows are restored to NaN after reconstruction.
    ...
    # mle requires n_samples >= n_features (sklearn hard constraint).
    # Fall back to a safe component count for small maps.
    mle_fallback_note: str | None = None
    _n_components = n_components
    if _n_components == "mle" and n_spectra < n_spectral:
        _n_components = max(1, n_spectra // 2)
        mle_fallback_note = (
            f"mle unavailable (n_spectra={n_spectra} < n_spectral={n_spectral}); "
            f"fell back to n_components={_n_components}"
        )

    pca = decomposition.PCA(n_components=_n_components, **(pca_kwargs or {}))
    ...
    if has_nan:
        cleaned_rows[nan_row_mask] = np.nan
    ...
    if mle_fallback_note:
        meta["mle_fallback"] = mle_fallback_note
