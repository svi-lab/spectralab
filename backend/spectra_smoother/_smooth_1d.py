...
def wavelet_smooth_1d(
    ...
    Decomposes the spectrum with a discrete wavelet transform, estimates the
    noise level from the finest detail coefficients, applies a universal
    threshold ``σ·√(2·log N)`` to all detail subbands, then reconstructs.
    ...
    y
        1-D array of intensities.
    wavelet
        Wavelet family name (e.g. ``"db4"``, ``"sym4"``, ``"coif3"``).
    level
        Decomposition depth.  ``None`` → maximum level for the signal length.
    threshold_mode
        ``"soft"`` (default) — shrinks coefficients toward zero, producing
        smoother output.  ``"hard"`` — zeroes small coefficients outright,
        better preserves sharp peaks.

    Returns
    -------
    np.ndarray
        Denoised spectrum, same length as ``y``, dtype float64.
    """
    try:
        import pywt
        ...
        raise ImportError(
            ...
            "conda install pywavelets  or  pip install PyWavelets"
        ) from exc
    ...
    # Estimate noise σ from finest detail band using the MAD estimator
    finest = coeffs[-1]
    sigma = float(np.median(np.abs(finest))) / 0.6745
    threshold = sigma * np.sqrt(2.0 * np.log(max(len(y), 2)))

    # Threshold all detail subbands (keep approximation coefficients intact)
    denoised_coeffs = [coeffs[0]] + [
        pywt.threshold(c, threshold, mode=threshold_mode) for c in coeffs[1:]
    ]

    reconstructed = pywt.waverec(denoised_coeffs, wavelet)
    # waverec may add one extra sample for odd-length inputs
    return reconstructed[: len(y)]
